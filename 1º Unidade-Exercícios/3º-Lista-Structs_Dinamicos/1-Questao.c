...
void alterarSal(funcionario *point, int tam){   //Função para alterar o salário de algum funcionário cadastrado.
    ...
    if(test == 'S' || test == 's'){
        ...
        printf("Digite o número de sistema: ");
        scanf("%d", &i);
        printf("Salário atual desse funcionário: R$%.2f\nQual é o novo salário? R$", point[i].salario);
...
void imprimir(funcionario *point, int tam){     //Função para imprimir o nome, idade, cargo, salário e identificador de todos os funcionários cadastrados.  
    int i;
